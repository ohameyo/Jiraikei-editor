...
- 拍立得模式下二次编辑腮红时，会把腮红选区从原图坐标映射到拍立得照片实际显示区域，修复预览选区和真实腮红位置不一致。
- 拍立得模式下修改腮红强度、移动或缩放选区后，会同步刷新拍立得内部 `photoCanvas`，避免继续显示确认拍立得时缓存下来的旧腮红。
- 腮红编辑模式下不再额外绘制 canvas 腮红区域预览，选区 DOM 改为轻量边框提示，避免移动或缩放后真实腮红与预览腮红叠成两层效果。

## 修改文件
- `src/app.js`
- `tasks/blush-manual-region.md`
- `tasks/polaroid-production.md`
- `TASKS.md`

## 验证结果
- `node --check src/app.js` 通过。
- 已用本地服务 `http://127.0.0.1:4181/` 打开页面，首屏无控制台 error。
- 已静态复核拍立得腮红编辑路径：overlay 选区、canvas 腮红预览和最终 `photoCanvas` 均使用同一套拍立得照片显示映射。
- 已静态复核腮红编辑模式：`drawBlushPreview()` 在 `blushEditMode` 下直接返回，避免选区编辑期间重复叠加一层粉色效果。
...
- 仍需在 Cloudflare 测试地址用真实手机图片复测：开启拍立得后再次编辑腮红选区、拖动/缩放选区、导出保存三条路径。
