...
- `getLipColorWeight()`：基于原图红/玫红色相、饱和度、明度和人脸/主体区域识别唇色候选，用于所有滤镜共享的唇妆保留。
- `applyTonePipeline()`：依次处理基础亮度/对比度/饱和度、HSL、绿色色块修正、唇色保留、肤色提白/腮红、色温/色调/雾感、叠色、黑发暗部保护和最终腮红叠染。
...
- 红色保真目前依赖 red HSL、肤色/唇色候选修正、低饱和补偿和共享唇色保留逻辑；没有独立 `redPreserve` 面板参数。
...
### 2026-04-30 / 全部滤镜 / 唇色保留不足
- 样张：用户反馈所有滤镜对唇色保留都不够。
- 修改前目标：水色、灰粉、黑白等低饱和预设会把唇妆压得过淡；原逻辑只有红色 HSL 和绿点修正，没有专门把唇色残色补回。
- 修改字段：新增 `getLipColorWeight()`；在 `applyTonePipeline()` 中 HSL 和绿色色块修正之后加入共享唇色恢复，按原图红/玫红色相、饱和度、明度和人脸/主体上下文计算权重，并根据全局去饱和和 master 去饱和强度提高恢复量。
- 修改后结果：目标是所有滤镜都能保留唇妆识别度，尤其水色、灰粉、黑白下不再把唇色完全压平；无脸框时只做弱保留，降低红色背景/配饰被误增强的风险。
- 通过样张：待用真实导入照片复测。
- 失败样张：待记录。
- 决策：候选合入；下一轮重点观察红色配饰、粉发、脸颊腮红是否被误判为唇色而过度回红。
- 验证：`node --check src/app.js` 通过。
...
- 增加所有滤镜共享的唇色保留逻辑，补回低饱和预设下被压掉的唇妆残色。
