...
- PC 端非滤镜滑杆保留拖动中实时提交；滤镜滑杆改为拖动中 CSS 乐观预览，避免桌面端 canvas 重绘阻塞滑杆。
...
- PC/移动端滤镜滑杆拖动中统一走 CSS 即时预览，不再拖动中写入 store 或触发 canvas 重渲染；松手后一次性提交准确渲染，优先保证滑杆跟手和预览实时。
- HSL 滑杆拖动中增加近似 CSS 预览，避免调色面板拖动时仍依赖高成本像素渲染。
- 黑白/灰粉等低叠加强度预设使用动态细步进，避免 `0.03` / `0.1` 这类小范围滑杆在移动端只剩少数几个档位。
