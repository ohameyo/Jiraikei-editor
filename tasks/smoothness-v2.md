...
- 增加画布右下角「加工中」标识：仅当一次真实 render 耗时超过阈值时短暂显示，不改变滑杆提交、渲染调度或腮红/图层交互路径。
- 调整「加工中」标识为全局等待提示：滤镜、腮红、贴纸、文字、马赛克等真实渲染耗时超过阈值时均可短暂显示；但直接拖动/缩放/旋转进行中会隐藏，避免影响跟手路径。
- 优化「加工中」出现时机：非轻量真实渲染前先点亮标识并让出一帧绘制机会，移动端滑杆松手提交、腮红/元素释放后的等待阶段可更早看到提示。
...
## 遗留风险
- 当前改动已通过静态检查，尚未在真机/浏览器中复测。
- 后续优化必须避免改变滤镜最终审美参数和页面整体视觉。
- 「加工中」已在部分真实渲染前预显示，但核心渲染仍是主线程同步任务；若需要进度级实时反馈，仍需后续拆分/异步化渲染管线。
