...
test('polaroid frame cards expose portrait and landscape orientation hooks', async () => {
  const app = await readFile(new URL('../../src/app.js', import.meta.url), 'utf8')
  const styles = await readFile(new URL('../../styles.css', import.meta.url), 'utf8')

  assert.match(app, /btn\.classList\.add\(orientation === 'portrait' \? 'is-portrait' : 'is-landscape'\)/)
